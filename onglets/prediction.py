    ...
    if st.button("🔮 Calculer la Prédiction", use_container_width=True):
        with st.spinner("Analyse en cours..."):
            try:
                ...
                patient_data["Tempsdesuivi"] = round(cleaned_pred, 1)  # Utilisation du bon nom de colonne
                patient_data["Deces"] = "OUI"  # Exemple d'ajout d'une autre donnée

                # 💾 Enregistrement automatique du patient
                save_new_patient(patient_data)  # Assurez-vous que cette fonction utilise la colonne correcte
